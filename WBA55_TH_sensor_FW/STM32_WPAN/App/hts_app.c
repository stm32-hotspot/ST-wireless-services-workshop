...
/* External variables --------------------------------------------------------*/
/* USER CODE BEGIN EV */
extern SHT40AD1B_Object_t Temp_Sensor;
...
static uint32_t HTS_FloatToIEEE11073(float temp_c);
...
void HTS_APP_Measurement(void)
{
  ...
  if (SHT40AD1B_TEMP_GetTemperature(&Temp_Sensor, &temp_c) != SHT40AD1B_OK)
  {
    LOG_INFO_APP("SHT40AD1B_TEMP_GetTemperature failed in HTS_APP_Measurement\n");
    return;
  }

  LOG_INFO_APP("Sensor temperature: %.2f C\n", temp_c);

  measurement = HTS_FloatToIEEE11073(temp_c);
...
static void HTS_APP_IntermediateTemperature(void)
{
  ...
  LOG_INFO_APP("Intermediate temperature: %.2f C\n", temp_c);

  /* Encode in IEEE11073 format */
  HTS_APP_Context.IntermediateTemperatureChar.MeasurementValue = HTS_FloatToIEEE11073(temp_c);
  ...
  {
	    LOG_INFO_APP("HTS_APP_IntermediateTemperature (IEEE11073 raw): 0x%08lX\n",
...
static uint32_t HTS_FloatToIEEE11073(float temp_c)
{
  /* mantissa in 0.01 �C units */
  int32_t mantissa = (int32_t)roundf(temp_c * 100.0f);
  int8_t exponent = -2;  /* 10^-2 */

  uint32_t u_mantissa = (uint32_t)(mantissa & 0x00FFFFFF); /* 24-bit two's complement */
  uint32_t u_exponent = (uint32_t)((uint8_t)exponent);     /* 8-bit two's complement */

  return (u_mantissa | (u_exponent << 24));
}
