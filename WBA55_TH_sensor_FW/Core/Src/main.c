...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
SHT40AD1B_IO_t Temp_Sensor_bus;
SHT40AD1B_Object_t Temp_Sensor;
/* USER CODE END 0 */
...
/* USER CODE BEGIN 4 */
void MX_Sensor_Init(void)
{
  Temp_Sensor_bus.Address = SHT40AD1B_I2C_ADDRESS;
  Temp_Sensor_bus.BusType = SHT40AD1B_I2C_BUS;
  Temp_Sensor_bus.DeInit = BSP_I2C1_DeInit;
  Temp_Sensor_bus.Delay = HAL_Delay;
  Temp_Sensor_bus.GetTick = BSP_GetTick;
  Temp_Sensor_bus.Init = BSP_I2C1_Init;
  Temp_Sensor_bus.DeInit = BSP_I2C1_DeInit;
  Temp_Sensor_bus.Read = BSP_I2C1_Recv;
  Temp_Sensor_bus.Write = BSP_I2C1_Send;

  if (SHT40AD1B_RegisterBusIO(&Temp_Sensor, &Temp_Sensor_bus) != SHT40AD1B_OK)
  {
    Error_Handler();
  }

  if(SHT40AD1B_Init(&Temp_Sensor) != SHT40AD1B_OK)
  {
    Error_Handler();
  }

  SHT40AD1B_TEMP_Enable(&Temp_Sensor);


}

void MX_Sensor_Deinit(void)
{
  Temp_Sensor_bus.Address = SHT40AD1B_I2C_ADDRESS;
  Temp_Sensor_bus.BusType = SHT40AD1B_I2C_BUS;
  Temp_Sensor_bus.DeInit = BSP_I2C1_DeInit;
  ...
  SHT40AD1B_TEMP_Disable(&Temp_Sensor);

  if(SHT40AD1B_DeInit(&Temp_Sensor) != SHT40AD1B_OK)
  {
    Error_Handler();
  }
}
